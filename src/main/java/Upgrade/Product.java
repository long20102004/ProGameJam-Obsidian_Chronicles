...
public class Product {
    ...
    public void update(){
        ...
        productBorder.width = WIDTH;
        productBorder.height = HEIGHT;
        isPicking = false;
        if (isMoved){
            isPicking = true;
            xDrawOffset = (int) (-0.05f * WIDTH);
            yDrawOffset = (int) (-0.05f * HEIGHT);
            productBorder.width = (int) (1.1f * WIDTH);
            productBorder.height = (int) (1.1 * HEIGHT);
        }
        if (isPressed){
            isPicking = true;
            xDrawOffset = (int) (0.05f * WIDTH);
            yDrawOffset = (int) (0.05f * HEIGHT);
            productBorder.width = (int) (0.9f * WIDTH);
            productBorder.height = (int) (0.9f * HEIGHT);
        }
    }
