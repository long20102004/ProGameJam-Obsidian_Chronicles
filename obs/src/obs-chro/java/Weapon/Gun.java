...
import static utilz.Constant.GUN.*;
...
public class Gun {
    ...
    private int typeAction = IDLE_GUN_1;
    ...
    private void initClass() {
        ...
        for (int i=0; i<guns.length; i++){
            for (int j=0; j<guns[0].length; j++){
                guns[i][j] = tmp.getSubimage(j * DEFAULT_WIDTH, i * DEFAULT_HEIGHT, DEFAULT_WIDTH, DEFAULT_HEIGHT);
    ...
    public void draw(Graphics g, float xLevelOffset,float yLevelOffset){
        ...
        if (gunslinger.isRight()) g2d.drawImage(guns[typeAction][drawIndex], (int) (xPos), (int) (yPos), WIDTH, HEIGHT, null);
        else g2d.drawImage(revGuns[typeAction][drawIndex], (int) (xPos), (int) (yPos), WIDTH, HEIGHT, null);
        drawBullet(g);
    }
    ...
    public void updateAngle(MouseEvent e) {
        ...
        if (gunslinger.isRight()) {
            dx = e.getX() - (double) xPos + (double) WIDTH / 2;
            dy = e.getY() - (double) yPos + (double) HEIGHT / 2;
        }
        else{
            dx = (double) xPos - e.getX() + (double) WIDTH / 2;
            dy = (double) yPos - e.getY() + (double) HEIGHT / 2;
        }
