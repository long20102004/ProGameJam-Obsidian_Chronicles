...
public class Spitter extends Enemy {
    ...
    private final int attackSight = TILE_SIZE * 10;
    private final int sight = 10 * attackSight;
    private final int xDrawOffset = (int) (15 * MODE);
    private final int yDrawOffset = (int) (10 * MODE);
    private ArrayList<ProjectTile> projectTileList = new ArrayList<>();
...
//            g.drawRect((int) ((int) hitbox.x - xLevelOffset), (int) ((int) hitbox.y - yLevelOffset), (int) hitbox.width, (int) hitbox.height);
    ...
    public void update(Game game) {
        ...
        updateAttackBox();
        for (ProjectTile projectTile : projectTileList){
            if (projectTile.isActive()) projectTile.update();
        }
    }
    ...
    private void handleAttackState(Game game) {
        ...
        if (!isAttacked && drawIndex == getType(ATTACK) / 2) {
            ...
            enemyAttack(game, Enemies.Spitter.damage, 0);
            isAttacked = true;
        }
    ...
    class ProjectTile {
        ...
        private void initClass() {
            border = new Rectangle2D.Float(xPos, yPos + 5, 32, 32);
        ...
        public void draw(Graphics g, float xLevelOffset, float yLevelOffset) {
            g.drawImage(animation[status][drawIndex], (int) ((int) border.x - xLevelOffset), (int) ((int) border.y - yLevelOffset), (int) border.width, (int) border.height, null);
        }
